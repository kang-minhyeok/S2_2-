...
"""
global_visited_cams = {}
def process_video_analysis(report_id: int, content: str = None, cams_to_process: list = None):
    global global_visited_cams
    if cams_to_process is None or cams_to_process == ["CAM_01"]:
        cams_to_process = ["CAM_01"]
        global_visited_cams[report_id] = set()

    if report_id not in global_visited_cams:
        global_visited_cams[report_id] = set()

    actual_cams = []
    for cam in cams_to_process:
        if cam not in global_visited_cams[report_id]:
            global_visited_cams[report_id].add(cam)
            actual_cams.append(cam)
        else:
            print(f" [SYSTEM]  {cam} 이미 수색한 구역입니다.")
    ...
    try:
        for cam_name in actual_cams:
            src_name = cam_to_file.get(cam_name)

            if not src_name or not os.path.exists(src_name):
                print(f" [경고] {cam_name} 파일이 없습니다. 건너뜁니다.")
                continue

            # "CAM_02" 같은 이름에서 숫자 '2'만 추출하여 채널 번호로 사용
            i = int(cam_name.split("_")[1])

            print(f" [{cam_name}] 분석 시작: {src_name} (Target: {target_color} {target_type})")
            emit_log(f"{cam_name} 채널 분석 시작...")
            ...
            while cap.isOpened():
                ...
                is_last_frame = (frame_count >= total_frames - 2) if total_frames > 0 else False
                ...
                if frame_count % 3 == 0:
                    ...
                    if results[0].boxes.id is not None:
                        ...
                        for j, obj_id in enumerate(ids):
                            ...
                            has_entered = prev.get("has_entered", False)
                                    ...
                                    if curr_feat_str:
                                        ...
                                        if reid_score >= 0.65:
                                            ...
                                            if prev_cam and curr_cam and latest_handover.exit_time:
                                                ...
                                                time_elapsed = max((datetime.datetime.now() - latest_handover.exit_time).total_seconds(), 0.1)
                                                if (dist / time_elapsed) <= 15.0:
                            ...
                            if is_reid_matched and is_exiting and not ex_sent:
                                ...
                                if obj_id in local_roi_buffer and local_roi_buffer[obj_id]:
                                    ...
                                    if extracted_feats:
                                        ...
                                        feat_str = json.dumps((avg_feat / norm(avg_feat)).tolist())

                                if not feat_str and target_saved_feat:
                                    feat_str = target_saved_feat

                                try:
                                    trigger_cam_name = f"CAM_0{i}"
                                    new_event = HandoverEvent(
                                        report_id=report_id, obj_id=int(obj_id),
                                        from_cam=trigger_cam_name, vx=float(vx), vy=float(vy),
                                    ...
                                    def schedule_camera_analysis(current_vx, current_vy, origin_cam):
                                        ...
                                        try:
                                            start_cam = local_db.query(CameraTopology).filter(CameraTopology.cam_name == origin_cam).first()
                                            all_cams = local_db.query(CameraTopology).filter(CameraTopology.cam_name != origin_cam).all()
                                            if not start_cam: return
                                    ...
                                    threading.Thread(target=schedule_camera_analysis, args=(vx, vy, trigger_cam_name)).start()
                    ...
                    if target_color == "" or target_color.lower() in data["color"].lower():
                        ...
                        if current_time - last_log_time > 1.5 and not data.get("ex_sent"):
                            if not target_saved_feat:
                                emit_log(f"MATCH: CAM 0{i}에서 {data['color']} 타겟 추적 중")
